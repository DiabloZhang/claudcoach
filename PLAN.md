...
- topic detection 只看最近用户发言，不把长篇教练回复塞进分类 prompt；分类模型返回坏 JSON 时，要用轻量关键词兜底，避免明显伤病对话卡在分类阶段而无法写教练笔记
- topic 结构化写入必须可观测：命中 topic 后的总结结果、是否写入、失败原因都要能在接口或模型调用日志里看到，不能只在后端静默吞异常
- injury topic 已明确但身体部位还不清楚时，先写入一条“未明确部位”的草稿记录，避免用户已经表达伤病但教练笔记为空；后续对话再逐步补充和合并
- 模型调用日志需要按动作分类，而不是全部记成 `extract`；至少区分 `chat`、`categorize_topic`、`summarize_injury`，方便定位“识别了 topic 但没有写教练笔记”的断点
...
- [x] 对话保持开放，coach 不主动结束和锁定输入；topic 整理不再绑定“结束对话”
- [x] 每 3 条用户消息触发一次 topic detection；只检测最近 3 条用户消息对应的上下文，如果最近 3 条没有某个 topic，就不再总结该 topic
- [x] topic 命中后做总结提炼时，最多读取最近 20 条用户消息对应的上下文，并结合现有 topic 记录更新结构化摘要
- [x] 用户最后一次发言 10 分钟后自动做一次延迟 topic detection 与总结提炼，补齐未满 3 轮但已经形成有效信息的对话
- [x] 对话应有存档和可回溯性，用户不能因为新对话或刷新而失去之前聊天上下文
- [x] Coach 页面增加可展开的模型调用日志侧边栏，展示本轮发送给模型的上下文、使用模型、模型原始返回与结构化提取结果，便于调试记忆和 topic 写入
...
- [x] Coach 页面支持用户选择 LLM provider 优先级，至少提供 Gemini 优先 / Claude 优先；后端仍可在首选 provider 失败时 fallback，但要向用户展示实际使用的模型
- [x] Coach 页面展示当前 conversation 已识别 topics，例如伤病、训练计划、睡眠，让用户知道哪些信息正在被记录
- [x] 增加教练笔记页面，先展示 `user_injury` 记录，帮助用户确认 coach 已经记下长期状态
- [x] 对话信息面板支持查看历史对话，并支持手动触发当前对话 topic / 教练笔记整理用于线上调试
