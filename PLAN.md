...
- 教练**主动**找用户，不等用户发问
- 交互：PC 默认打字，手机默认语音（参考豆包）
- 不做表单，所有信息通过对话收集，Claude 提取后存结构化字段

#### 语音技术方案
- STT / TTS：字节火山引擎（中文识别准，音色自然）
- 音色：用户可选预设音色
- 声音克隆（模仿运动明星等）：nice-to-have，Phase 4 之后
...
赛季 Season（目标 + 赛季总结）
  └─ 比赛 Race（每场一个总结）
  └─ 月总结 × N（教练在月末 sleep_time 自动生成）
       └─ 周总结 × N（教练在周日 sleep_time 自动生成）
            └─ 每日一行日志（教练在 sleep_time 写，新活动/对话后更新）
                  └─ 对话记录（原始）
```
...
#### 教练任务队列（CoachTask）
- 教练头顶显示 ❓ 未解决任务
- 任务类型：goal_missing / injury_followup / plan_check / race_debrief 等
...
- [x] 是否支持 Garmin / Wahoo 等其他数据源 → **决定支持，由 whz 负责（Phase W2）**
- [x] 是否需要用户账号系统 → **决定做 SaaS 多用户，由 whz 负责（Phase W1）**
